...
Email: ${generatedData.email}`;
  ...
  return <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <Button onClick={generateData} disabled={isGenerating} className="bg-gradient-to-r from-green-500 to-green-600 hover:from-green-600 hover:to-green-700 text-white font-bold">
      ...
      {generatedData && <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          {/* Dados Pessoais */}
          <Card className="bg-gray-800/50 border-gray-700">
            <CardHeader>
            ...
            <CardContent className="space-y-3">
              {[{
            label: 'Nome',
            value: generatedData.nome
          }, {
            label: 'CPF',
            value: generatedData.cpf
          }, {
            label: 'RG',
            value: generatedData.rg
          }, {
            label: 'Gênero',
            value: generatedData.genero
          }, {
            label: 'Nascimento',
            value: generatedData.nascimento
          }, {
            label: 'Telefone',
            value: generatedData.telefone
          }, {
            label: 'Email',
            value: generatedData.email
          }].map((item, index) => <div key={index} className="flex items-center justify-between p-3 bg-gray-700/50 rounded-lg">
                  <div>
                    <p className="text-gray-400 text-sm">{item.label}</p>
                    <p className="text-white font-mono">{item.value}</p>
                  </div>
                  <Button size="sm" variant="outline" onClick={() => copyField(item.value, item.label)} className="border-gray-600 text-gray-400 hover:bg-green-500 hover:border-green-500 hover:text-white">
          ...
          <Card className="bg-gray-800/50 border-gray-700">
            ...
            <CardContent className="space-y-3">
              {[{
            label: 'Rua',
            value: generatedData.endereco.rua
          }, {
            label: 'Bairro',
            value: generatedData.endereco.bairro
          }, {
            label: 'Cidade',
            value: generatedData.endereco.cidade
          }, {
            label: 'Estado',
            value: generatedData.endereco.estado
          }, {
            label: 'CEP',
            value: generatedData.endereco.cep
          }].map((item, index) => <div key={index} className="flex items-center justify-between p-3 bg-gray-700/50 rounded-lg">
                  <div>
                    <p className="text-gray-400 text-sm">{item.label}</p>
                    <p className="text-white font-mono">{item.value}</p>
              ...
              <Button onClick={copyAll} className="w-full mt-4 bg-green-500 hover:bg-green-600 text-white font-bold">
